...
def detect_outliers(
    ...
    """
    Detect outliers in DataFrame (numeric columns).
    ...
    for col in numeric_cols:
        if method == "iqr":
            col_outliers = detect_outliers_iqr(df[col], multiplier=threshold)
        else:  # zscore
            col_outliers = detect_outliers_zscore(df[col], threshold=threshold)
