...
def load_all_data(data_dir=None):
    ...
    file_map = {
        ...
        'sales':        ('Sales.csv', ['Date'], '\t'),  # TAB-DELIMITED | DD-MM-YYYY HH:MM
    ...
    for key, (fname, date_cols, delimiter) in file_map.items():
        ...
        if not os.path.exists(fpath):
            ...
            continue
        try:
            # Special handling for Sales.csv: tab-delimited with DD-MM-YYYY HH:MM format
            if key == 'sales':
            ...
            # Drop index columns (Unnamed: 0)
            if 'Unnamed: 0' in data[key].columns:
                data[key] = data[key].drop('Unnamed: 0', axis=1)
            
            print(f"  [OK] {key:20s} → {data[key].shape[0]:>8,} rows × {data[key].shape[1]:>3} cols  ({fname})")
...
def validate_mmix_data(data):
    """
    Validate all datasets for common issues.
    ...
    if 'monthly' in data:
        ...
        if 'Date' in m.columns:
            ...
            if dups > 0:
                msg = f"⚠️  Monthly: {dups} duplicate dates found"
                issues.append(msg)
                print(f"  {msg}")
            else:
                print("  [OK] No duplicate dates in monthly data")

        # Negative values in spend/revenue
        numeric_cols = m.select_dtypes(include=[np.number]).columns
        for c in numeric_cols:
            neg_count = (m[c] < 0).sum()
            if neg_count > 0:
                msg = f"⚠️  Monthly: {neg_count} negative values in '{c}'"
                issues.append(msg)
                print(f"  {msg}")
        ...
        if 'Date' in m.columns and 'total_gmv' in m.columns:
            ...
            if len(low_months) > 0:
                ...
                msg = f"⚠️  Monthly: Anomalously low GMV in: {dates}"
                issues.append(msg)
                print(f"  {msg}")
    ...
    if 'transactions' in data:
        ...
        if 'gmv_new' in tx.columns:
            neg = (tx['gmv_new'] < 0).sum()
            if neg > 0:
                msg = f"⚠️  Transactions: {neg} negative GMV values"
                issues.append(msg)
                print(f"  {msg}")
    ...
    if 'special_sales' in data:
        ...
        expected_cols = ['Date', 'Sales Name']
        for c in expected_cols:
            if c not in ss.columns:
                # Try alternate names
                alt = [col for col in ss.columns if c.lower().replace(' ', '_') in col.lower()
                       or c.lower().replace(' ', '') in col.lower()]
                if alt:
                    msg = f"ℹ️  SpecialSale: Column '{c}' not found, but found '{alt[0]}'"
                    issues.append(msg)
                    print(f"  {msg}")
                else:
...
def national_trends(monthly_df, save_dir='outputs/plots'):
    ...
    if 'total_gmv' in df.columns:
        axes[0, 0].plot(df['Date'], df['total_gmv'] / 1e7, marker='o', color='#2196F3', linewidth=2)
    ...
    if 'total_Units' in df.columns:
        axes[0, 1].plot(df['Date'], df['total_Units'] / 1e3, marker='s', color='#4CAF50', linewidth=2)
    ...
    if 'total_Discount' in df.columns:
        axes[1, 0].plot(df['Date'], df['total_Discount'] / 1e7, marker='^', color='#FF5722', linewidth=2)
...
def category_breakdown(monthly_df, save_dir='outputs/plots'):
    ...
    unit_cols = [c for c in df.columns if c.startswith('Units_')]
    ...
    for col, cat in zip(rev_cols, categories):
        axes[0].plot(df['Date'], df[col] / 1e7, marker='o', label=cat, linewidth=2)
    ...
    summary = pd.DataFrame({
        'Category': categories,
        'Avg Monthly Revenue (Cr)': [df[c].mean() / 1e7 for c in rev_cols],
        'Avg Monthly Units': [df[c].mean() for c in unit_cols] if unit_cols else [None]*len(rev_cols),
        'Revenue Std Dev (Cr)': [df[c].std() / 1e7 for c in rev_cols],
    }).round(2)
    print("\n📊 Category Summary:")
...
def media_investment_analysis(monthly_df, save_dir='outputs/plots'):
    ...
    for ch in channels:
        vals = pd.to_numeric(df[ch], errors='coerce') / 1e7
    ...
    axes[1].barh(list(avg_spend.keys()),
                 [v / 1e7 for v in avg_spend.values()],
    ...
    share_df = pd.DataFrame({
        ...
        'Avg Spend (Cr)': [f"{v/1e7:.2f}" for v in avg_spend.values()],
...
def correlation_analysis(monthly_df, save_dir='outputs/plots'):
...
def special_sale_impact(transactions_df, special_sales_df, save_dir='outputs/plots'):
    ...
    sale_name_col = None
    for candidate in ['Sales Name', 'Sales_Name', 'Sales_name', 'sales_name']:
        if candidate in ss.columns:
            sale_name_col = candidate
            break
    if not sale_name_col:
        print("[WARN] Cannot find sale name column in SpecialSale. Skipping.")
        return

    # Detect GMV column in transactions
    gmv_col = None
    for candidate in ['gmv_new', 'GMV', 'gmv']:
    ...
    axes[0].boxplot([nonsale_gmv / 1e5, sale_gmv / 1e5] if len(sale_gmv) > 0 else [nonsale_gmv / 1e5],
                    labels=['Non-Sale Days', 'Sale Days'][:1 + (len(sale_gmv) > 0)],
                    patch_artist=True,
    ...
    print(f"  Non-Sale Days → Avg GMV: ₹{nonsale_gmv.mean()/1e5:.1f}L, Median: ₹{np.median(nonsale_gmv)/1e5:.1f}L")
    if len(sale_gmv) > 0:
        print(f"  Sale Days     → Avg GMV: ₹{sale_gmv.mean()/1e5:.1f}L, Median: ₹{np.median(sale_gmv)/1e5:.1f}L")
...
def channel_overlap_analysis(monthly_df, save_dir='outputs/plots'):
    """Compare traditional vs digital channel spend and their relationship with revenue."""
    df = monthly_df.copy()
    if not pd.api.types.is_datetime64_any_dtype(df['Date']):
        df['Date'] = pd.to_datetime(df['Date'])
    df = df.sort_values('Date')
    ...
    df['digital_share'] = np.where(
        df['total_spend'] > 0,
        df['digital_spend'] / df['total_spend'] * 100,
        0
    )
    ...
    axes[0].bar(df['Date'], df['traditional_spend'] / 1e7, label='Traditional', color='#3F51B5')
    axes[0].bar(df['Date'], df['digital_spend'] / 1e7,
                bottom=df['traditional_spend'] / 1e7, label='Digital', color='#00BCD4')
    ...
    if 'total_gmv' in df.columns:
        axes[2].scatter(df['traditional_spend'] / 1e7, df['total_gmv'] / 1e7,
                        label='Traditional', s=100, color='#3F51B5', edgecolors='white')
        axes[2].scatter(df['digital_spend'] / 1e7, df['total_gmv'] / 1e7,
    ...
    print(f"  Avg Traditional Spend: ₹{df['traditional_spend'].mean()/1e7:.1f} Cr/month")
    print(f"  Avg Digital Spend:     ₹{df['digital_spend'].mean()/1e7:.1f} Cr/month")
...
def nps_revenue_analysis(monthly_df, save_dir='outputs/plots'):
    ...
    ax1 = axes[0]
    ax2 = ax1.twinx()
    ax1.plot(df['Date'], df['total_gmv'] / 1e7, 'b-o', label='GMV', linewidth=2)
    ...
    axes[1].scatter(nps_clean['NPS'], nps_clean['total_gmv'] / 1e7, s=120, c='#9C27B0', edgecolors='white')
    if len(nps_clean) > 2:
        z = np.polyfit(nps_clean['NPS'], nps_clean['total_gmv'] / 1e7, 1)
        p = np.poly1d(z)
...
def run_full_eda(data_dir=None, save_dir=None):
    ...
    Returns all data, classifications, validation issues, and correlation matrix.
    """
    # Auto-detect paths from script location if not provided
    ...
    print("=" * 60)
    print("[START] STARTING EDA PIPELINE — E-Commerce MMIX")
    print("=" * 60)
    ...
    if 'monthly' in data:
        ...
        cat_summary = category_breakdown(data['monthly'], save_dir)

        # --- Step 7: Media Investment ---
        print("\n💰 STEP 7: Media Investment Analysis...")
        spend_summary = media_investment_analysis(data['monthly'], save_dir)

        # --- Step 8: Correlation Analysis ---
        print("\n🔗 STEP 8: Correlation Analysis...")
        corr_matrix = correlation_analysis(data['monthly'], save_dir)
    else:
        print("[WARN] Monthly data not loaded. Skipping steps 5-8.")
        corr_matrix = None
...
if __name__ == "__main__":
    data, classifications, issues, corr_matrix = run_full_eda(
        data_dir=None,  # Auto-detect project root
        save_dir=None   # Auto-detect project root
    )
